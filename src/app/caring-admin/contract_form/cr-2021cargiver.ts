...
export class doc2021Giver {

    public create2021Caregiver(name: string, center: string, time: string, address: string, place: string, phone: string, start: string, contract: string): Document {
        ...
        document.addSection({
            children: [
                ...
                this.createTitletext("4. 임 금 (기본시급 :11,200원)"),
                this.createText("① 임금은 기본급(시간 당 8,720원), 차량유지비, 법정수당(주휴수당(시간 당 1,744원), 연차수당(미사용 연차에 대해 시간 당 503원), 기타수당 및 식대(시간 당 233원)) 등으로 구성함."),
                this.createText("- 가족요양, 일반목욕은 기본시급을 기준으로 산정하되, 추가할 금액을 기타수당 등으로 보전할 수 있음."),
    ...
    public createTitle(): Paragraph{
        return new Paragraph({
            ...
            children: [
                new TextRun({
                    text: "요양보호사 근로계약서 (단시간 근로자 _ 시급제)",
                    bold: true,
                    size: 25,
                }),
            ],
