...
export class ConsultListComponent implements OnInit {
    ...
    load($event: any) {
        if(this.searchOn==='기본'){
            ...
            this.api.consult.getHomepageReservation('홈페이지 예약').subscribe(count => this.homepage_reservation = count);
            ...
            if(this.state==='' || this.state===null){
                ...
                if(this.searchType==='통합'){
                    if(this.search.length>=1){
                        this.api.consult.getStateSearchConsult( this.state, this.search, $event, 7).subscribe( data => this.data = data );
                    }
                } else if(this.searchType==='현재상황'){
                    if(this.search.length>=1){
                        this.api.consult.getStateNowConsult( this.state, this.search, $event, 7).subscribe( data => this.data = data );
                    }
                } else if(this.searchType==='상담내용'){
                    if(this.search.length>=1){
                        this.api.consult.getStateMemoConsult( this.state, this.search, $event, 7).subscribe( data => this.data = data );
                    }
                }
            }
        }
    ...
    getSearch(searchType: string, search: string){
        if(this.state==='' || this.state===null){
            ...
            if(searchType==='통합'){
                this.api.consult.getStateSearchConsult( this.state, search, 0, 7).subscribe( data => this.data = data );
            } else if(searchType==='현재상황'){
                this.api.consult.getStateNowConsult( this.state, search, 0, 7).subscribe( data => this.data = data );
            } else if(searchType==='상담내용'){
                this.api.consult.getStateMemoConsult( this.state, search, 0, 7).subscribe( data => this.data = data );
            }
        }
        ...
        for (let i=0;i<this.meta_data.content.length;i++){
            if(this.meta_data.content[i] != null) {
                ...
                if (this.meta_data.content[i].come === '인터넷') {
                    ...
                    } else if (this.meta_data.content[i].city === '경기') {
                    ...
                    } else if (this.meta_data.content[i].city === '강원') {
                    ...
                    } else if (this.meta_data.content[i].city === '제주') {
                        this.jeju_leaflet++;
                        if (this.meta_data.content[i].state === '가족_서비스시작') {
                ...
                if (this.meta_data.content[i].lastModifiedDate != null && this.meta_data.content[i].lastModifiedDate != '' && this.meta_data.content[i].lastModifiedDate.substring(0, 10) === this.today) {
                    ...
                    if (create.toISOString().substring(0, 10) === this.today) {
                        ...
                        if (this.meta_data.content[i].state === '가족_진행중(1~4급)') {
                            ...
                            if(this.meta_data.content[i].now != null && this.meta_data.content[i].now != '' && this.meta_data.content[i].now.includes('서류')) {
                                this.call_document++
                            }
                        } else if (this.meta_data.content[i].state === '가족_등급신청중') {
                            ...
                            // if (this.meta_data.content[i].now.includes('추후')) {
                            //     this.call_after++
                            // }
                            //
